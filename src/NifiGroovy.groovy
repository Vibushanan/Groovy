...
cli.with {
  ...
  a longOpt: 'authorization-key', 'bearer token to authenticate on server',
	args:1, argName:'key', type:String.class
...
def getNumberOfQueuedFiles() {
  ...
  numberOfQueuedFiles = queued[0].replaceAll(",", "").toInteger()
...
/*
This methos takes process group as input. It iterates throgh each sub process group inside root level process group.
*/
def loadProcessors(pgId,processors){
  resp = nifi.get(path: "controller/process-groups/$pgId/processors")
  assert resp.status == 200
  // construct a quick map of "procName -> [id, fullUri, comment, processGroupID]"
  def tmp = resp.data.processors.collectEntries {
	[(it.name): [it.id, it.uri, it.comments, pgId]]
  }
  processors.putAll(tmp)
  
  // get the processgroups inside current process group
  resp = nifi.get(path: "controller/process-groups/$pgId/process-group-references")
  assert resp.status == 200
  def recProcessGroups = resp.data.processGroups

  // iterate over each sub process groups and make recursive call for each process group
  recProcessGroups?.each{
	loadProcessors(it.id,processors)
  }
...
/**
 ...
 - locate the processor according to the nesting structure in YAML. If you have
	nested process group withing a root process group, then you MUST specifi all the
	processors at root level process group. YML should not have nested level process groups
	inside root process group.
   (intentionally not using 'search') to pick up a specific PG->Proc
 - This script can not handle duplicate processor name. It will not throw any
	any error. Instead, it just cofigures only one of them.
...
def handleProcessGroup(Map.Entry pgConfig) {
  ...
  loadProcessors(pgId,processors)
  pgConfig.value.processors.each { proc ->
	...
	pgId = processors[proc.key][3] // getting the corresponding pgID for the processor
...
assert user : 'Authorization user must be provided'
assert pass : 'Authorization password must be provided'
